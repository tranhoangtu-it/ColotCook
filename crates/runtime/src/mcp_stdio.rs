...
/// Backend for a managed MCP server — either a local stdio process or a remote HTTP client.
#[derive(Debug)]
enum McpServerBackend {
    /// Stdio-based: spawns a child process, communicates via stdin/stdout JSON-RPC.
    Stdio(Option<McpStdioProcess>),
    /// HTTP-based: sends JSON-RPC requests via HTTP POST to a remote URL.
    Http(crate::mcp_http::McpHttpClient),
}
...
impl ManagedMcpServer {
    fn new_stdio(bootstrap: McpClientBootstrap) -> Self {
        Self {
            bootstrap,
            backend: McpServerBackend::Stdio(None),
            initialized: false,
        }
    }
    ...
    fn new_http(
        bootstrap: McpClientBootstrap,
        http_client: crate::mcp_http::McpHttpClient,
    ) -> Self {
        Self {
            bootstrap,
            backend: McpServerBackend::Http(http_client),
            initialized: false,
        }
    }
}
...
impl McpServerManager {
    ...
    pub async fn from_servers_async(servers: &BTreeMap<String, ScopedMcpServerConfig>) -> Self {
        ...
        for (server_name, server_config) in servers {
            ...
            match server_config.transport() {
                McpTransport::Stdio => {
                    managed_servers
                        .insert(server_name.clone(), ManagedMcpServer::new_stdio(bootstrap));
                }
                McpTransport::Http | McpTransport::Sse => {
                    // Both HTTP and SSE use McpHttpClient (request-response).
                    // SSE servers operate in polling mode — true streaming is
                    // not yet implemented.
                    // Extract the remote transport from the bootstrap config.
                    let remote_transport = match &bootstrap.transport {
                        McpClientTransport::Http(t) | McpClientTransport::Sse(t) => t.clone(),
                        _ => {
                            unsupported_servers.push(UnsupportedMcpServer {
                                server_name: server_name.clone(),
                                transport: server_config.transport(),
                                reason: "internal transport mismatch".to_string(),
                            });
                            continue;
                        }
                    };
                    match crate::mcp_http::McpHttpClient::new(server_name, &remote_transport).await
                    {
                        Ok(http_client) => {
                            managed_servers.insert(
                                server_name.clone(),
                                ManagedMcpServer::new_http(bootstrap, http_client),
                            );
                        }
                        Err(err) => {
                            unsupported_servers.push(UnsupportedMcpServer {
    ...
    pub async fn call_tool(
        ...
        let response = match &mut server.backend {
            McpServerBackend::Stdio(ref mut process_slot) => {
                ...
                Self::run_process_request(
                    ...
                    "tools/call",
                    timeout_ms,
                    process.call_tool(request_id, params),
                )
                .await
            }
            McpServerBackend::Http(ref http_client) => {
                http_client.call_tool(request_id, params).await
            }
        };
    ...
    pub async fn shutdown(&mut self) -> Result<(), McpServerManagerError> {
        ...
        for server_name in server_names {
            ...
            match &mut server.backend {
                ...
                McpServerBackend::Http(_) => {
                    // HTTP clients are stateless, nothing to shut down.
                }
            }
    ...
    fn server_process_exited(&mut self, server_name: &str) -> Result<bool, McpServerManagerError> {
        let server = self.server_mut(server_name)?;
        match &mut server.backend {
            McpServerBackend::Stdio(ref mut process_slot) => match process_slot.as_mut() {
                Some(process) => Ok(process.has_exited()?),
                None => Ok(false),
            },
            McpServerBackend::Http(_) => Ok(false), // HTTP clients don't "exit"
        }
    }
            ...
            let response = match &mut server.backend {
                McpServerBackend::Stdio(ref mut process_slot) => {
                    ...
                    Self::run_process_request(
                        ...
                        process.list_tools(request_id, Some(params)),
                    )
                    .await?
                }
                McpServerBackend::Http(ref http_client) => {
                    http_client.list_tools(request_id, params).await?
                }
            };
    ...
    async fn reset_server(&mut self, server_name: &str) -> Result<(), McpServerManagerError> {
        ...
        match &mut server.backend {
            ...
            McpServerBackend::Http(_) => {
                // HTTP clients are stateless; just reset the initialized flag.
            }
        }
    ...
    async fn ensure_server_ready(
        ...
        let mut attempts = 0;
        loop {
            // For Stdio backends, spawn the process if it's not running yet.
            {
                let server = self.server_mut(server_name)?;
                if let McpServerBackend::Stdio(ref mut process_slot) = server.backend {
                    if process_slot.is_none() {
                        *process_slot = Some(spawn_mcp_stdio_process(&server.bootstrap)?);
                        server.initialized = false;
                    }
                }
            }
            ...
            // Send the initialize request via the appropriate backend.
            let server = self.server_mut(server_name)?;
            let response = match &mut server.backend {
                McpServerBackend::Stdio(ref mut process_slot) => {
                    let process = process_slot.as_mut().ok_or_else(|| {
                ...
                McpServerBackend::Http(ref http_client) => http_client.initialize(request_id).await,
            };
            ...
            // For HTTP backends, send the initialized notification.
            {
                let server = self.server_mut(server_name)?;
                if let McpServerBackend::Http(ref http_client) = server.backend {
                    let _ = http_client.send_initialized().await;
                }
            }
...
mod tests {
    ...
    #[tokio::test]
    async fn spawns_stdio_process_and_round_trips_io() {
    ...
    #[tokio::test]
    async fn round_trips_initialize_request_and_response_over_stdio_frames() {
    ...
    #[tokio::test]
    async fn write_jsonrpc_request_emits_content_length_frame() {
        let script_path = write_jsonrpc_script();
        let transport = script_transport(&script_path);
        let mut process = McpStdioProcess::spawn(&transport).expect("spawn transport directly");
    ...
    #[tokio::test]
    async fn given_lowercase_content_length_when_initialize_then_response_parses() {
        let script_path = write_jsonrpc_script();
    ...
    #[tokio::test]
    async fn given_mismatched_response_id_when_initialize_then_invalid_data_is_returned() {
        let script_path = write_jsonrpc_script();
        let transport = script_transport_with_env(
            &script_path,
    ...
    #[tokio::test]
    async fn direct_spawn_uses_transport_env() {
        let script_path = write_echo_script();
    ...
    #[tokio::test]
    async fn lists_tools_calls_tool_and_reads_resources_over_jsonrpc() {
    ...
    #[tokio::test]
    async fn surfaces_jsonrpc_errors_from_tool_calls() {
        let script_path = write_mcp_server_script();
        let transport = script_transport(&script_path);
        let mut process = McpStdioProcess::spawn(&transport).expect("spawn fake mcp server");
    ...
    #[tokio::test]
    async fn manager_discovers_tools_from_stdio_config() {
        ...
        let mut manager = McpServerManager::from_servers_async(&servers).await;
    ...
    #[tokio::test]
    async fn manager_routes_tool_calls_to_correct_server() {
        let script_path = write_manager_mcp_server_script();
        let root = script_path.parent().expect("script parent");
    ...
    #[tokio::test]
    async fn manager_times_out_slow_tool_calls() {
        let script_path = write_mcp_server_script();
        let root = script_path.parent().expect("script parent");
    ...
    #[tokio::test]
    async fn manager_surfaces_parse_errors_from_tool_calls() {
        let script_path = write_mcp_server_script();
        let servers = BTreeMap::from([(
            "broken".to_string(),
            ScopedMcpServerConfig {
    ...
    #[tokio::test]
    async fn given_child_exits_after_discovery_when_calling_twice_then_second_call_succeeds_after_reset(
    ) {
        let script_path = write_manager_mcp_server_script();
        let root = script_path.parent().expect("script parent");
    ...
    #[tokio::test]
    async fn given_initialize_hangs_once_when_discover_tools_then_manager_retries_and_succeeds() {
        let script_path = write_manager_mcp_server_script();
        let root = script_path.parent().expect("script parent");
    ...
    #[tokio::test]
    async fn given_tool_call_disconnects_once_when_calling_twice_then_manager_resets_and_next_call_succeeds(
    ) {
        let script_path = write_manager_mcp_server_script();
        let root = script_path.parent().expect("script parent");
    ...
    fn manager_records_unsupported_non_stdio_servers_without_panicking() {
        ...
        // HTTP is now supported, so only SDK and WebSocket should be unsupported.
        assert_eq!(unsupported.len(), 2);
        assert_eq!(unsupported[0].server_name, "sdk");
        assert_eq!(unsupported[1].server_name, "ws");

        // HTTP server should be in the managed servers map.
        assert!(manager.servers.contains_key("http"));
    }

    #[tokio::test]
    async fn manager_shutdown_terminates_spawned_children_and_is_idempotent() {
        let script_path = write_manager_mcp_server_script();
        let root = script_path.parent().expect("script parent");
        let log_path = root.join("alpha.log");
        let servers = BTreeMap::from([(
    ...
    #[tokio::test]
    async fn manager_reuses_spawned_server_between_discovery_and_call() {
        let script_path = write_manager_mcp_server_script();
        let root = script_path.parent().expect("script parent");
        let log_path = root.join("alpha.log");
        let servers = BTreeMap::from([(
    ...
    #[tokio::test]
    async fn manager_reports_unknown_qualified_tool_name() {
        let script_path = write_manager_mcp_server_script();
        let root = script_path.parent().expect("script parent");
        let log_path = root.join("alpha.log");
        let servers = BTreeMap::from([(
