...
impl OpenAiCompatConfig {
    ...
    /// Returns true if this provider allows empty/missing API keys (e.g., local Ollama).
    #[must_use]
...
impl OpenAiCompatClient {
    ...
    pub fn from_env(config: OpenAiCompatConfig) -> Result<Self, ApiError> {
        // Try primary env var first, then fall back to alternates (e.g., GOOGLE_API_KEY for Gemini)
        let api_key = config
            .credential_env_vars()
            .iter()
            .find_map(|var| read_env_non_empty(var).ok().flatten());

        match api_key {
            Some(key) => Ok(Self::new(key, config)),
            None if config.allows_empty_key() => {
                // Ollama and other local providers don't require API keys
                Ok(Self::new(String::new(), config))
            }
