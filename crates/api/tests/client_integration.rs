...
use serde_json::json;
use colotcook_telemetry::{ClientIdentity, MemoryTelemetrySink, SessionTracer, TelemetryEvent};
...
async fn send_message_applies_request_profile_and_records_telemetry() {
    ...
    let client = AnthropicClient::new("test-key")
        ...
        .with_extra_body_param("metadata", json!({"source": "colotcook-code"}))
    ...
    let body: serde_json::Value =
        serde_json::from_str(&request.body).expect("request body should be json");
    assert_eq!(body["metadata"]["source"], json!("colotcook-code"));
    assert_eq!(
        body["betas"],
        json!([
