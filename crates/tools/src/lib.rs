...
use colotcook_api::ToolDefinition;
use colotcook_plugins::PluginTool;
use colotcook_runtime::{
    execute_bash, grep_search, BashCommandInput, GrepSearchInput, PermissionMode,
};
...
pub fn execute_tool(name: &str, input: &Value) -> Result<String, String> {
    match name {
        ...
        "TodoWrite" => from_value::<types::TodoWriteInput>(input).and_then(run_todo_write),
        "Skill" => from_value::<types::SkillInput>(input).and_then(run_skill),
        "Agent" => from_value::<types::AgentInput>(input).and_then(run_agent),
        "ToolSearch" => from_value::<types::ToolSearchInput>(input).and_then(run_tool_search),
        "NotebookEdit" => from_value::<types::NotebookEditInput>(input).and_then(run_notebook_edit),
        ...
        "REPL" => from_value::<types::ReplInput>(input).and_then(run_repl),
        "PowerShell" => from_value::<types::PowerShellInput>(input).and_then(run_powershell),
...
#[allow(clippy::needless_pass_by_value)]
fn run_web_fetch(input: types::WebFetchInput) -> Result<String, String> {
    to_pretty_json(web_tools::execute_web_fetch(&input)?)
}

#[allow(clippy::needless_pass_by_value)]
fn run_web_search(input: types::WebSearchInput) -> Result<String, String> {
    to_pretty_json(web_tools::execute_web_search(&input)?)
}

fn run_todo_write(input: types::TodoWriteInput) -> Result<String, String> {
    to_pretty_json(session_tools::execute_todo_write(input)?)
}

fn run_skill(input: types::SkillInput) -> Result<String, String> {
    to_pretty_json(session_tools::execute_skill(input)?)
}

fn run_agent(input: types::AgentInput) -> Result<String, String> {
    to_pretty_json(agent_tools::execute_agent(input)?)
}

fn run_tool_search(input: types::ToolSearchInput) -> Result<String, String> {
    to_pretty_json(search_tools::execute_tool_search(input))
}

fn run_notebook_edit(input: types::NotebookEditInput) -> Result<String, String> {
    to_pretty_json(execution_tools::execute_notebook_edit(input)?)
}

fn run_sleep(input: types::SleepInput) -> Result<String, String> {
    to_pretty_json(execution_tools::execute_sleep(input)?)
}

fn run_brief(input: types::BriefInput) -> Result<String, String> {
    to_pretty_json(execution_tools::execute_brief(input)?)
}

fn run_config(input: types::ConfigInput) -> Result<String, String> {
    to_pretty_json(system_tools::execute_config(input)?)
}

fn run_enter_plan_mode(input: types::EnterPlanModeInput) -> Result<String, String> {
    to_pretty_json(system_tools::execute_enter_plan_mode(input)?)
}

fn run_exit_plan_mode(input: types::ExitPlanModeInput) -> Result<String, String> {
    to_pretty_json(system_tools::execute_exit_plan_mode(input)?)
}

fn run_structured_output(input: types::StructuredOutputInput) -> Result<String, String> {
    to_pretty_json(system_tools::execute_structured_output(input)?)
}

fn run_repl(input: types::ReplInput) -> Result<String, String> {
    to_pretty_json(execution_tools::execute_repl(input)?)
}

fn run_powershell(input: types::PowerShellInput) -> Result<String, String> {
    to_pretty_json(execution_tools::execute_powershell(input).map_err(|error| error.to_string())?)
}
...
mod tests {
    ...
    fn temp_path(name: &str) -> PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        std::env::temp_dir().join(format!("colotcook-tools-{unique}-{name}"))
    }
    ...
    fn web_search_handles_generic_links_and_invalid_base_url() {
        ...
        std::env::set_var("COLOTCOOK_WEB_SEARCH_BASE_URL", "://bad-base-url");
    ...
    fn todo_write_persists_and_returns_previous_state() {
        ...
        std::env::set_var("COLOTCOOK_TODO_STORE", &path);
    ...
    fn agent_persists_handoff_metadata() {
        ...
        std::env::set_var("COLOTCOOK_AGENT_STORE", &dir);
